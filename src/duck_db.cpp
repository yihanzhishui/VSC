...
const char *errorMessage =
    "> your input is invalid,print \".help\" for more infomation!\n";
...
void printHelpMess() {
    cout << "******************************************************************"
         ...
         << "  .reset 							delete "
            "db file;"
         << endl
         << "  insert db {index} {name} {age} {email}; 			insert "
            "record;"
         << endl
         << "  delete from db where id ={index}; 			"
            "	delete record;"
         << endl
         << "  update db {name} {age} {email} where id={index}; 	"
            "	update a record;"
         << endl
         << "  select * from db where id={index}; 			"
            "	search a record by index;"
         << endl
         << "  select * from db where id in({minIndex},{maxIndex}); 	"
            "	search records between indexs;"
         << endl
         << "******************************************************************"
            "***************************"
         << endl
...
void selectCommand() {
    ...
    while (true) {
        ...
        if (strcmp(userCommand, ".exit") == 0) {
            ...
            int okNum =
                sscanf(userCommand, "insert db %d %s %d %s;", keyIndex,
            ...
            if (okNum < 3) {
                ...
                int return_code =
                    insertRecord(duck_db_ptr, keyIndex, insertData);
                finishTime = clock();

                if (return_code == 0) {
                    // cout << ">insert\n";
                    cout << "> executed insert index:" << *keyIndex
            ...
            int *keyIndex = new int;
            int okNum =
                sscanf(userCommand, "delete from db where id=%d;", keyIndex);
            ...
            if (!strstr(userCommand, "=")) {
                int i_start, i_end;
                int okNum =
                    sscanf(userCommand, "select * from db where id in(%d,%d);",
                           &i_start, &i_end);
                ...
                int *keyIndex = new int;
                int okNum = sscanf(userCommand, "select * from db where id=%d;",
                                   keyIndex);
                ...
                if (okNum < 1) {
                    ...
                    int return_code =
                        searchRecord(duck_db_ptr, keyIndex, return_val);
            ...
            int okNum = sscanf(userCommand, "update db %s %d %s where id=%d;",
                               updateData->name, &(updateData->age),
                               updateData->email, keyIndex);
            ...
            if (okNum < 3) {
                ...
                startTime = clock();
                int return_code =
                    updateRecord(duck_db_ptr, keyIndex, updateData);
...
int main(int argc, char *argv[]) { initialSystem(); }
