...
:anchor:[**图解：什么是B树？一文读懂B-树 - 知乎**](https://zhuanlan.zhihu.com/p/146252512)
...
1. 在 B-树中查找给定关键字的记录，若查找成功，则插入操作失败；否则将新记录作为空指针p插人到查找失败的叶子结点的上一层结点（由q指向）中。
2. 若插入新记录和空指针后，q 指向的结点的关键字个数未超过 m-1，则插入操作成功，否则转入步骤 3。
3. 以该结点的第 $\lceil m/2 \rceil$ 个关键字 $K_{\lceil m/2 \rceil}$ 为拆分点，将该结点分成3个部分：$K_{\lceil m/2 \rceil}$ 左边部分、$K_{\lceil m/2 \rceil}$、$K_{\lceil m/2 \rceil}$右边部分。$K_{\lceil m/2 \rceil}$左边部分仍然保留在原结点中； $K_{\lceil m/2 \rceil}$右边部分存放在一个新创建的结点（由p指向）中；关键字值为 $K_{\lceil m/2 \rceil}$ 的记录和指针p插入到q的双亲结点中。因q的双亲结点增加一个新的记录，所以必须对q的双亲结点重复 2 和 3 的操作，依次类推，直至由q指向的结点是根结点，转入步骤 4。
4. 由于根结点无双亲，则由其分裂产生的两个结点的指针p和q，以及关键字为 $K_{\lceil m/2 \rceil}$ 的记录构成一个新的根结点。此时，B-的高度增加 1。

```cpp
Status InsertBTree(BTree &T, KeyType K, BTree q, int i) {
    x = K;
    ap = nullptr;
    finished = false; // x表示新插入的关键字
    while (q && !finished) {
        Insert(q, i, x, ap); // 将x和ap分别插入到q->key[i+1]和q->ptr[i+1]
        if (q->key_num < m) finished = true; // 插入完成
        else {               // 分裂结点*q
            s =⌈m / 2⌉;
            split(q, s, ap);
            x = q->key[s];
            // 将q->key[s+1..m],q->ptr[s..m]和q->reptr[s+1..m]移入新结点*ap
            q = q->parent;
...
:anchor:[**B树、B+树详解 - 博客园**](https://www.cnblogs.com/lianzhilei/p/11250589.html)
...
:anchor:[**一篇文章搞懂什么是散列表? - 知乎**](https://zhuanlan.zhihu.com/p/343658094)
...
平方取中法的适用情况：不能事先了解关键字的所有情况，或难千直接从关键字中找到取值 较分散的几位。

###### c. 折叠法

折叠法的适用情况：适合于散列地址的位数较少，而关键字的位数较多，且难千直接从关键 字中找到取值较分散的几位。

###### d. 除留余数法

除留余数法计算简单，适用范围非常广，是最常用的构造散列函数的方法。
...
1. 线性探测：最坏情况下时间复杂度是O(n)
2. 二次探测：二次探测就是和线性探测类似，线性探测的每次步长是1，二次探测的步长是 hash(key)+0、(hash(key)+1)^2^、(hash(key)+2)^2^。
3. 双重散列：双重探测就是使用的是一组散列函数 hash~1~(key)、hash~2~(key)、hash~3~(key) ... 我们现使用第一个散列函数，如果位置占用了，再使用第二个散列函数，依次类推。不管使用那个探测方法，随着空闲位置越来越少，散列冲突的概率都会大大提高，所以为了保证散列表的操作效率，尽可能保证散列表中有一定比例的空闲位置，我们采用**装载因子**来表示空位的多少，**装载因子的计算公式是：装载因子 = 填入的数据个数 / 散列表的长度**，装载因子越大，说明空闲位置越少，散列冲突越大。
...
2. 循环n-1次，每次使用顺序查找法，查找r[i]\(i=2,···,n) 在已排好序的序列r[1···i-1]中的插入位置，然后将r[i]插入表长为i-1的有序序列r[1···i-1]，直到将r[n]插入表长为n-1的有序序列r[1..n-l]，最后得到一个表长为n的有序序列。
...
void InsertSort(SqList &L) {
    for (i = 2; i <= L.length; i++)
        if (L.r[i].key < L.r[i - 1].key) { // "<"，需将r[i]插入有序子表
            ...
            L.r[j + 1] = L.r[0]; // 将r[0]即原r[i]，插人到正确位置
        }                        // if
}
```

算法时间复杂度为 O(n^2^)。空间复杂度为 O(1)。

##### B. 折半插入排序

直接插入排序采用顺序查找法查找当前记录在已排好序的序列中的插入位置，“查找”操作可利用“折半查找”来实现，由此进行的插入排序称之为折半插入排序。

算法步骤：

1. 设待排序的记录存放在数组r[1..n]中，r[1]是一个有序序列。
2. 循环n-1次，每次使用折半查找法，查找r[i]\(i=2,...,n)在已排好序的序列r[1..i-1]中的插入位置，然后将r[i]插入表长为i-1的有序序列r[1..i-1]，直到将r[n]插入表长为n-1的有序序列r[l … n-1]，最后得到一个表长为n的有序序列。
...
希尔排序实质上是采用分组插入的方法。先将整个待排序记录序列分割成几组，从而减少参与 直接插入排序的数据量，对每组分别进行直接插入排序，然后增加每组的数据量，重新分组。 这样 当经过几次分组排序后，整个序列中的记录 “基本有序” 时，再对全体记录进行一次直接插入排序。

希尔对记录的分组，不是简单地”逐段分割”，而是将相隔某个“增量”的记录分成一组。

1. 第一趟取增量d~1~(d<n)把全部记录分成d~1~个组，所有间隔为d~1~的记录分在同一组，在各个组中进行直接插入排序。
2. 第二趟取增量d~2~(d~2~<d~1~)，重复上述的分组和排序。
3. 依次类推，直到所取的增量d~t~=1(d~t~<d~t~-1<···<d~2~<d~1~)，所有记录在同一组中进行直接插人排序为止。
...
void ShellSort(SqList &L, int dt[], int t) {
    for (k = 0; k < t; k++)
        ShellInsert(L, dt[k]); // 一起增量为dt[t]的希尔插入排序
}
```
...
void BubbleSort(SqList &L) {
    m = L.length - 1;
    flag = 1; // flag用来标记某一趟排序是否发生转换
    while ((m > 0) && (flag == 1)) {
        flag = 0; // flag置为0，如果本趟排序发生了交换
        for (j = 1; j <= m; j++)
...
1. 选择待排序表中的第一个记录作为枢轴，将枢轴记录暂存在r[O]的位置上。附设两个指针low和high，初始时分别指向表的下界和上界（第一趟时，low=1;high=L.length;)。
2. 从表的最右侧位置依次向左搜索，找到第一个关键字小于枢轴关键字pivotkey的记录，将其移到low处。具体操作是：当low<high时，若high所指记录的关键字大于等于pivotkey，则向左移动指针high（执行操作--high）；否则将high所指记录移到low所指记录。
...
void QSort(SqList &L, int low, int high) {
    if (low < high) { // 长度大于1
        pivotloc = Partition(
            L, low, high); // 将L.r[low..high]一分为二，pivotloc是枢轴位置
        QSort(L, low, pivotloc - 1);  // 对左子表递归排序
        QSort(L, pivotloc + 1, high); // 对左子表递归排序
    }
}
...
时间复杂度 O($nlog_2n$)。最好空间复杂度 O(n)，最坏时间复杂度 O(n)。
...
1. csh：unix下默认终端
   ...
   ```bash
    sudo apt install csh // Linux安装 csh
   ```
...
1. 常见执行Linux指令格式
   ...
   ```bash
   命令名称 [命令参数] [命令对象] // 严格区分大小写
   ```

   命令名称：动词；

   命令参数：用于对命令进行调整，让“修改”过的命令能更好地贴合工作需求。分为长格式（完整命令名称）、短格式（缩写）。
...
- `echo`命令-回显
  ...
  ```bash
  echo PWD // 回车过后，直接输出PWD
  echo $PWD // 回车过后，输出变量PWD的值：当前的文件夹
  ```
...
- `date`命令
  ...
  ```cpp
   date "+%Y-%m-%d %H:%M:%S" // 2023-04-18 21:01:53
  ```
...
- `pidof`：用于查询某个指定服务进程的`PID`号码值，语法格式为“pidof [参数] 服务名称”。

- `kill`：用于终止某个指定`PID`值的服务进程，语法格式为“kill [参数] 进程的 PID”。
...
- `ipconfig`：用于获取网卡配置与网络状态等信息，语法格式为“ifconfig [参数] [网络设备]”。

  主要查看的就是网卡名称、inet 参数后面的IP地址、ether参数后面的网卡物理地址（又称为MAC地址），以及RX、TX 的接收数据包与发送数据包的个数及累计流量。

- `uname`：用于查看系统内核版本与系统架构等信息，英文全称为“unix name”，语法格 式为“uname [-a]”。

- `uptime`：用于查看系统的负载信息，输入该命令后按回车键执行即可。

- `free`：用于显示当前系统中内存的使用量信息，语法格式为“free [-h]”。

  -h：易读模式。如果不使用-h（易读模式）查看内存使用量情况，则默认以KB为单位。

  ```bash
   ~  free                                       ok | root@YICHANG  09:00:46 PM
                total        used        free      shared  buff/cache   available
  Mem:        8070004      154324     7392256          68      523424     7681296
  Swap:       2097152        2584     2094568
   ~  free -h                                    ok | root@YICHANG  09:02:02 PM
                total        used        free      shared  buff/cache   available
  Mem:          7.7Gi       150Mi       7.0Gi       0.0Ki       511Mi       7.3Gi
  Swap:         2.0Gi       2.0Mi       2.0Gi
  ```

- `who`：用于查看当前登入主机的用户终端信息，输入该命令后按回车键执行即可。

- `last`：用于调取主机的被访记录，输入该命令后按回车键执行即可。

- `ping`：用于测试主机之间的网络连通性，语法格式为“ping [参数] 主机地址”。
...
- `history`：用于显示执行过的命令历史，语法格式为“history [-c]”。

  -c：清空之前的历史。

- `sosreport`：用于收集系统配置及架构信息并输出诊断文档，输入该命令后按回车键执行即可。

#### 5. 查找定位文件命令

- `pwd`：用于显示用户当前所处的工作目录，输入该命令后按回车键执行即可。

- `cd`：用于切换当前的工作路径，语法格式为“cd [参数] [目录]”。

  ```bash
  yihanzhishui@yihanzhishui:~$ cd /etc
  yihanzhishui@yihanzhishui:/etc$ cd /bin
  yihanzhishui@yihanzhishui:/bin$ cd -
  /etc
  yihanzhishui@yihanzhishui:/etc$ cd ~
  yihanzhishui@yihanzhishui:~$ pwd
  /home/yihanzhishui                                      
  ```

- `ls`：用于显示目录中的文件信息，语法格式为“ls [参数] [文件名称]”。

- `tree`：用于以树状图的形式列出目录内容及结构，输入该命令后按回车键执行即可。
...
- `locate`：用于按照名称快速搜索文件所对应的位置，语法格式为“locate 文件名称”。

  部分Linux系统并未安装locate

  ```bash
  sudo apt-get install mlocate //安装locate
  ```

- `whereis`：用于按照名称快速搜索二进制程序（命令）、源代码以及帮助文件所对应的位置，语法格式为“whereis 命令名称”。
...
- `cat`：用于查看纯文本文件（内容较少的），语法格式为“cat [参数] 文件名称”。

  cat 命令后面追加一个-n 参数，显示行号。

- `more`：用于查看纯文本文件（内容较多的），语法格式为“more [参数] 文件名称”。

- `head`：用于查看纯文本文件的前N行，语法格式为“head [参数] 文件名称”。

- `tail`：用于查看纯文本文件的后N行或持续刷新文件的最新内容，语法格式为“tail [参数] 文件名称”。

- `tr`：用于替换文本内容中的字符，语法格式为“tr [原始字符] [目标字符]”。
...
- `grep`：令用于按行提取文本内容，语法格式为“grep [参数] 文件名称”。
...
- `cut`：用于按“列”提取文本内容，语法格式为“cut [参数] 文件名称”

- `diff`：用于比较多个文件之间内容的差异，英文全称为“different”，语法格式为“diff [参数] 文件名称A文件名称B”。

- `uniq`：用于去除文本中连续的重复行，英文全称为“unique”，语法格式为“uniq [参数] 文件名称”。
...
- `touch`：用于创建空白文件或设置文件的时间，语法格式为“touch [参数] 文件名称”。
  ...
  ```bash
   touch -d "2020-05-04 15:44" test // -rw-r--r--  1 root root    28 May  4  2020 test
  ```

- `mkdir`：令用于创建空白的目录，英文全称为“make directory”，语法格式为“mkdir [参数] 目录名称”。

  ```bash
   ~/test  mkdir linuxprobe // 创建linuxprobe文件夹                                   
   ~/test  cd linuxprobe               
   ~/t/linuxprobe  mkdir -p a/b/c  // 创建嵌套文件夹
   ~/t/linuxprobe  ls                                           
  a
   ~/t/linuxprobe  tree                                         
  .
...
- `tar`：用于对文件进行打包压缩或解压，语法格式为“tar 参数 文件名称”。Linux 系统中，主要使用的是.tar、.tar.gz 或.tar.bz2格式压缩包。
...
可概括为：把前一个命令原本要输出到屏幕的信息当作后一个命令的标准输入。

```bash
grep /sbin/nologin /etc/passwd | wc -l // 26
...
可以使用 export 命令将其提升为全局变量，执行 unset 命令可以把它取消掉。
...
1. 编写简单文档
   ...
   ```bash
   vim test // 使用vim编辑test
   进入test过后，按下i或者(insert)键进行编辑
   编辑完成过后按下ESC键退出编辑模式，进入命令模式，按下:wq保存退出
   ```
...
2. 配置软件仓库
   ...
   - 使用 Vim 编辑器创建一个名为 rhel8.repo 的新配置文件（文件名称可随意，但 后缀必须为.repo），逐项写入下面的配置参数并保存退出。

     > **仓库名称**：具有唯一性的标识名称，不应与其他软件仓库发生冲突。
     > **描述信息(name)**：可以是一些介绍性的词，易于识别软件仓库的用处。
     > **仓库位置(baseurl)**：软件包的获取方式，可以使用FTP或HTTP下载，也可以是本地的文件（需要在后面添file参数)。
...
2. 接收用户的参数
   ...
   // 执行脚本
   bash test.sh one two three four five six
   // 输出
   当前脚本名称为test.sh
...
3. 判断用户的参数
   ...
   ```bash
   [ -d /etc/fstab ] // 判断/etc/fstab是否为一个目录类型的文件
   echo $? // $?显示上一条语句的返回值
   ```
...
一次性计划任务只执行一次，一般用于临时的工作需求。可以用at命令实现这种功能，只需要写成“at 时间”的形式就行。

如果想要查看已设置好但还未执行的一次性计划任务，可以使用at -l命令；要想将其删除，可以使用“atrm 任务序号”。
...
使用 at 命令来设置一次性计划任务时，默认采用的是交互式方法。

创建、编辑计划任务的命令为crontab -e，查看当前计划任务的命令为crontab -l，删除某条计划任务的命令为crontab -r。
...
1. `id`：使用id命令查看一个名称为root的用户信息：

   ```bash
   id root // 命令
   uid=0(root) gid=0(root) groups=0(root) // 输出
   ```

2. `useradd`：用于创建新的用户账户，语法格式为“useradd [参数] 用户名”。
   ...
   useradd user_test1
   id user_test1 // 命令
   uid=1000(user_test1) gid=1000(user_test1) groups=1000(user_test1) // 输出
   
   useradd -d /home/linux -u 8888 -s /sbin/nologin user_test2
   id user_test2 // 命令
   uid=8888(user_test2) gid=8888(user_test2) groups=8888(user_test2) // 输出
   ```

3. `groupadd`：用于创建新的用户组，语法格式为“groupadd [参数] 群组名”。

   ```bash
   groupadd ronny // 增加ronny用户组
   ```

4. `usermod`：用于修改用户的属性，英文全称为“user modify”，语法格式为“usermod [参数] 用户名”。
   ...
   ```bash
   usermod -G root user_test1 // 将user_test1加入到root用户组
   usermod -u 6666 user_test1 // 修改user_test1的UID为6666
   id user_test1
   uid=6666(user_test1) gid=1000(user_test1) groups=1000(user_test1),0(root)
   ```

5. `passwd`：用于修改用户的密码、过期时间等信息，语法格式为“passwd [参数] 用户名”。

   普通用户只能使用passwd命令修改自己的系统密码，而root管理员则有权限修改其他所有人的密码。root管理员在 Linux系统中修改自己或他人的密码时不需要验证旧密码。

   | 参数    | 作用                                                   |
   | ------- | ------------------------------------------------------ |
   | -l      | 锁定用户，禁止其登录                                   |
   | -u      | 解除锁定，允许用户登录                                 |
   | --stdin | 通过标准输入修改用户密码，如 echo "NewPassWord"        |
   | -d      | 使该用户可用空密码登录系统                             |
   | -e      | 强制用户在下次登录时修改密码                           |
   | -S      | 显示用户的密码是否被锁定，以及密码所采用的加密算法名称 |

6. `userdel`：用于删除已有的用户账户，英文全称为“user delete”，语法格式为“userdel [参数] 用户名”。
...
文件的可读、可写、可执行权限的英文全称分别是read、write、execute，可以简写为 r、 w、x，亦可分别用数字4、2、1 来表示，文件所有者、文件所属组及其他用户权限之间无关联。

![image-20230421151231117](https://raw.githubusercontent.com/yihanzhishui/PicGo/img/image-20230421151231117.png)
...
2. SGID
   ...
   > 是参考SUID而设计的，不同点在于执行程序的用户获取的不再是文件所有者的临时权限，而是获取到文件所属组的权限。

   当对目录进行设置时，则是让目录内新创建的文件自动继承该目录原有用户组的名称。

   > 在某个目录中创建的文件自动继承该目录的用户组（只可以对目录进行设置）。

   `chmod`：用于设置文件的一般权限及特殊权限，英文全称为change mode，语法格式为“chmod [参数] 文件名”。

   `chown`：令用于设置文件的所有者和所有组，英文全称为change own，语法格式为“chown 所有者:所有组文件名”。

   > `chmod `和`chown `命令是用于修改文件属性和权限的最常用命令，它们还有一个特别的共性，就是针对目录进行操作时需要加上大写参数-R来表示递归操作，即对目录内所有的文件进行整体操作。
...
3. SBIT
   ...
   ```bash
   cd ~
   mkdir linux //创建linux文件夹
   chmod -R o+t linux/ //修改linux文件夹权限
   ls -ld linux/
   drwxr-xr-t 2 root root 4096 Apr 21 15:32 linux/
   ```
...
1. `chattr`：用于设置文件的隐藏权限，英文全称为change attributes，语法格式为“chattr [参数] 文件名称”。
...
2. `lsattr`：用于查看文件的隐藏权限，英文全称为list attributes，语法格式为“lsattr [参数] 文件名称”。

#### 5. 文件访问控制列表

1. `setfacl`：用于管理文件的ACL权限规则，英文全称为“set files ACL”，语法格式为“setfacl [参数] 文件名称”。
...
2. `getfacl`：用于查看文件的ACL权限规则，英文全称为“get files ACL”，语法格式为“getfacl [参数] 文件名称”。

#### 5. `su`命令和`sudo`服务

`su`命令可以解决切换用户身份的需求，使得当前用户在不退出登录的情况下，顺畅地切换到其他用户。

从root管理员=>普通用户：**不需要**密码验证；而从普通用户=>root管理员：**需要**进行密码验证
...
担心直接修改配置文件会出现问题，则可以使用`sudo`命令提供的`visudo`命令来配置用户权限。

`visudo`命令用于编辑、配置用户`sudo`的权限文件，语法格式为“visudo [参数]”。

使用visudo命令配置权限文件时，其操作方法与Vim编辑器中用到的方法完全一致，因此在编写完成后记得在末行模式下保存并退出。

## 五、存储结构与管理硬盘

![image-20230421162311537](https://raw.githubusercontent.com/yihanzhishui/PicGo/img/image-20230421162311537.png)
...
![image-20230421162637038](https://raw.githubusercontent.com/yihanzhishui/PicGo/img/image-20230421162637038.png)

#### 2. 挂载硬件设备

1. `mount`：用于挂载文件系统，格式为“mount 文件系统 挂载目录”。挂载是在使用硬件设备前所执行的最后一步操作。
...
2. `df`：用于查看已挂载的磁盘空间使用情况，英文全称为“disk free”，语法格式为“df -h”。-h参数可进位。

3. `umount`：用于卸载设备或文件系统，英文全称为“un mount”，语法格式为“umount [设备文件/挂载目录]”。

   `lsblk `命令用于查看已挂载的磁盘的空间使用情况，英文全称为“list block id”，输入该命令后按回车键执行即可。

#### 3. 添加硬盘设备

1. `fdisk`：用于新建、修改及删除磁盘的分区表信息，英文全称为“format disk”，语法格式为“fdisk 磁盘名称”。
...
2. `du`：用查看分区或目录所占用的磁盘容量大小，英文全称为“disk usage”，语法格式为“du -sh 目录名称”。
...
1. `xfs_quota`：用于管理设备的磁盘容量配额，语法格式为“xfs_quota [参数] 配额 文件系统”。

2. `edquota`：用于管理系统的磁盘配额，英文全称为“edit quota”，语法格式为“edquota [参数] 用户名”。
...
![image-20230421223254117](https://raw.githubusercontent.com/yihanzhishui/PicGo/img/image-20230421223254117.png)

#### 7. 软硬方式链接

> **软链接**：也叫符号链接，仅仅包含所链接文件的名称和路径，很像一个**记录地址的标签**。当原始文件被删除或移动后，新的链接文件也会随之失效，不能被访问。可以针对文件、目录设置软链接，跨文件系统进行链接也不是问题。
>
> **硬链接**：可以将它理解为一个“指向原始文件block的指针”，系统会创建出一个与原来一模一样的inode信息块。所以，**硬链接文件与原始文件其实是一模一样的，只是名字不同**。每添加一个硬链接，该文件的inode个数就会增加1；而且只有当该文件的inode个数为0时，才算彻底将它删除。换言之，由于硬链接实际上是指向原文件block的指针，因此即便原始文件被删除，依然可以通过硬链接文件来访问。
>
> ![image-20230422204646145](https://raw.githubusercontent.com/yihanzhishui/PicGo/img/image-20230422204646145.png)

`ln`：令用于创建文件的软硬链接，英文全称为“link”，语法格式为“ln [参数]原始文件名 链接文件名”。
...
:anchor:[Linux磁盘管理系列-LVM和RAID-博客园 (cnblogs.com)](https://www.cnblogs.com/qiuyu666/p/11912825.html)
