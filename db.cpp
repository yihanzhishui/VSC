  ...
  public:
    ...
    uint32_t *internal_node_child(uint32_t child_num) {
        ...
        if (child_num > num_keys) {
            std::cout << "Tried to access child_num " << child_num
    ...
    uint32_t *internal_node_key(uint32_t key_num) {
        return internal_node_cell(key_num) +
...
uint32_t Node::get_node_max_key() {
    if (get_node_type() == NODE_LEAF) {
        ...
        return *internal_node->internal_node_key(
            *internal_node->internal_node_num_keys() - 1);
    }
}
...
void Pager::print_tree(uint32_t page_num, uint32_t indentation_level) {
    ...
    case (NODE_LEAF):
        ...
        for (uint32_t i = 0; i < num_keys; i++) {
            ...
            std::cout << "- " << *((LeafNode *)node)->leaf_node_key(i)
                      << std::endl;
        }
        break;
    case (NODE_INTERNAL):
        ...
        for (uint32_t i = 0; i < num_keys; i++) {
            ...
            indent(indentation_level + 1);
            std::cout << "- key "
                      << *((InternalNode *)node)->internal_node_key(i)
                      << std::endl;
        }
...
void Cursor::leaf_node_split_and_insert(uint32_t key, Row &value) {
    ...
    */
    for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
        ...
        LeafNode destination =
            destination_node.leaf_node_cell(index_within_node);
        ...
        if (i == cell_num) {
            ...
            *destination_node.leaf_node_key(index_within_node) = key;
            ...
            memcpy(destination.get_node(), old_node.leaf_node_cell(i - 1),
                   LEAF_NODE_CELL_SIZE);
        } else {
            memcpy(destination.get_node(), old_node.leaf_node_cell(i),
                   LEAF_NODE_CELL_SIZE);
        }
    }
...
class DB {
  ...
  public:
    DB(const char *filename) { table = new Table(filename); }
    void start();
    void print_prompt();
...
PrepareResult DB::prepare_insert(std::string &input_line,
                                 Statement &statement) {
...
ExecuteResult DB::execute_insert(Statement &statement) {
    ...
    cursor->leaf_node_insert(statement.row_to_insert.id,
